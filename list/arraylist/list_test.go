...
func TestList_Remove(t *testing.T) {
	...
	referSize := int(float32(capacity) * thresholdShrink)

	for list.size-1 >= referSize {
		list.Remove(0)
	}
	assert.Equal(t, capacity, cap(list.elements))

	list.Remove(0)
	assert.True(t, cap(list.elements) < capacity)
}
...
func TestList_Insert(t *testing.T) {
	...
	assert.Equal(t, []int{0}, list.elements)

	list.Insert(0, 1)
	assert.Equal(t, 2, list.size)
	assert.Equal(t, 2, len(list.elements))
	assert.Equal(t, []int{1, 0}, list.elements)

	list.Insert(0, 3, 2)
	assert.Equal(t, 4, list.size)
	assert.Equal(t, 4, len(list.elements))
	assert.Equal(t, []int{3, 2, 1, 0}, list.elements)
}
...
func TestList_Set(t *testing.T) {
	...
	assert.Equal(t, 1, list.size)
	assert.Equal(t, 1, len(list.elements))
	assert.Equal(t, []int{1}, list.elements)
}
